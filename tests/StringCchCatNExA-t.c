...
void testDestEnd(){
	...
	strcpy(dest, "foo");
	ok(SUCCEEDED(StringCchCatNExA(dest, 11, "Data", 5, &destEnd, NULL, 0)),
	...
	ok(SUCCEEDED(StringCchCatNExA(dest, 11, "", 1, &destEnd, NULL, 0)),
			"Test calculation of destEnd "
	...
	strcpy(dest, "TEST: ");
	ok(StringCchCatNExA(dest, 11, "longer string", 14, &destEnd, NULL, 0) ==
...
void testRemaining(){
	...
	strcpy(dest, "Str");
	ok(SUCCEEDED(StringCchCatNExA(dest, 11, "Safe", 5, NULL, &remaining, 0)),
	...
	ok(SUCCEEDED(StringCchCatNExA(dest, 11, "", 1, NULL, &remaining, 0)),
			"Test calculation of remaining space "
			"while appending an empty string.");
	is_string("StrSafe", dest,
			"Result of appending an empty string.");
	...
	strcpy(dest, "TEST: ");
	ok(StringCchCatNExA(dest, 11, "too long string", 16, NULL, &remaining, 0) ==
			STRSAFE_E_INSUFFICIENT_BUFFER,
			"Test calculation of remaining space "
			"while appending a too long string.");
...
void testFlags(){
	...
	strcpy(dest, "test");
	ok(SUCCEEDED(StringCchCatNExA(dest, 11, NULL, 10, NULL, NULL,
	...
	strcpy(dest, "TEST: ");
	ok(FAILED(StringCchCatNExA(dest, 11, "too much data", 14, NULL, NULL,
	...
	strcpy(dest, "data");
	ok(FAILED(StringCchCatNExA(dest, 11, "Also too much", 14, NULL, NULL,
	...
	strcpy(dest, "data");
	ok(FAILED(StringCchCatNExA(dest, 11, "Won't fit in dest", 18, NULL, NULL,
