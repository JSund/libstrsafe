...
void testDestEnd(){
	...
	ok(SUCCEEDED(StringCchPrintfExW(dest, 11, &destEnd, NULL, 0, L"Data")),
	...
	ok(SUCCEEDED(StringCchPrintfExW(dest, 11, &destEnd, NULL, 0, L"")),
			"Test calculation of destEnd "
...
void testRemaining(){
	...
	ok(SUCCEEDED(StringCchPrintfExW(dest, 11, NULL, &remaining, 0, L"STR")),
	...
	ok(SUCCEEDED(StringCchPrintfExW(dest, 11, NULL, &remaining, 0, L"")),
			"Test calculation of remaining space "
			"while printing empty string.");
	is_wstring(L"", dest,
			"Result of printing empty string.");
	...
	ok(StringCchPrintfExW(dest, 11, NULL, &remaining, 0, L"too long string")
			== STRSAFE_E_INSUFFICIENT_BUFFER,
			"Test calculation of remaining space "
			"while printing a too long string.");
...
void testFlags(){
	...
	ok(SUCCEEDED(StringCchPrintfExW(dest, 11, NULL, NULL,
	...
	ok(SUCCEEDED(StringCchPrintfExW(dest, 11, NULL, NULL,
					STRSAFE_FILL_BEHIND_NULL | '@', L"testing")),
	...
	ok(FAILED(StringCchPrintfExW(dest, 11, NULL, NULL,
...
int main(void){
	...
	ok(SUCCEEDED(StringCchPrintfExW(dest, 11, NULL, NULL, 0, L"test")),
